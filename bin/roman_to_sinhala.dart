...
String toFuzzy(String input) {
  ...
  return toVel(input)
      ...
      .replaceAll(r'll', 'l')
      .replaceAll(r'ss', 's')
      .replaceAllMapped(r'([kgcjtdpb])[kgcjtdpb]{0,1}h*', (match) => match.group(0) ?? '');
}
...
String toSin(String input) {
  ...
  while (i < input.length) {
    ...
    if (vi1 != null) {
    ...
    } else if (conj[i1 + i2] != null) {		// two character match
      output += conj[i1+i2]!;
      i += 2;
      if(cons[i3] != null) {
        output += '්';
    ...
    } else if (sinhala[i1] != null && i1 != 'a') {		// one character match except a
      output += sinhala[i1]!;
      i++;
      ...
      output += i1;
      i++;
      if (vowel[i2] != null) {  // word-beginning vowel marker
        output += vowel[i2]!;
        i++;
      }
...
String toDeva(input) {
  ...
  input = input.toLowerCase().replace(r'ṁ', 'ṃ');
...
String fromThai(String input) {
  ...
  return input
      .replaceAllMapped(r'([อกขคฆงจฉชฌญฏฐฑฒณตถทธนปผพภมยรลฬวสห])(?!ฺ)', (match) => match.group(0) ?? '')
      .replaceAllMapped(r'([เโ])([อกขคฆงจฉชฌญฏฐฑฒณตถทธนปผพภมยรลฬวสหฺฺ]+a)', (match) => '${match.group(1) ?? ''}${match.group(0) ?? ''}')
